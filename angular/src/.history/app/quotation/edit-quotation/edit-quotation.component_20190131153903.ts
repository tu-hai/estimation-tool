...
export class EditQuotationComponent implements OnInit {
  ...
  delWorkItem(id: number): void {
    abp.message.confirm(
        ...
        (result: boolean) => {
            if (result) {
                // this.quotationservice.delWorkItem(id)
                //     .subscribe(() => {
                //         this.quoctationById = this.quoctationById.filter(u => u !== this.quoctationById);
                //         abp.notify.info('Deleted: ');
                //     });
                this.quotationservice.delWorkItem(id).subscribe(data => {
                  this.quoctationById = data;
                  this.quoctationById = this.quoctationById.filter();
                  abp.notify.info('Deleted');
                })
              }
        }
