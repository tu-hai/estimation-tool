...
export class EditQuotationComponent implements OnInit {
        ...
        (result: boolean) => {
            if (result) {
                this.quotationservice.delWorkItem(id).subscribe(data => {
                  ...
                  console.log('rest', this.quoctationById.wordItemDTOList);
  ...
  show(id: number): void {
    this.quotationservice.getQuoctationById(id).subscribe(data => {
      ...
      console.log('quotation data', this.quoctationById, this.quoctationById.id);
      this.active = true;
      this.modal.show();
      //this.quoctationById = Array.of(this.quoctaptionById);
    });
  }
